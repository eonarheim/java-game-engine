...
public class Image implements Drawable {
	...
	public static BufferedImage loadSubImage(BufferedImage spriteSheetImage,
		...
		image = spriteSheetImage.getSubimage((horizontalOffset) * width,
	...
	public static Image loadSubImageWithTexture(String ref, int width,
		...
		BufferedImage image = null;
		try {
			image = ImageIO.read(Animation.class.getClassLoader()
		 ...
		 * + moduloWidth);
		 */
		BufferedImage tmp = Image.loadSubImage(image, width, height,
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, get2Fold(tmp.getWidth()),
				get2Fold(tmp.getHeight()), 0, srcPixelFormat, GL_UNSIGNED_BYTE,
				textureBuffer);
	...
	public static Image loadImage(String ref) {
		...
		URL[] urls2 = ((URLClassLoader) Animation.class.getClassLoader())
				.getURLs();
		for (URL url : urls2) {
